				...
				<view class="uni-list-cell info-box" hover-class="uni-list-cell-hover" @click="selectValue(value)">
					<view class="uni-media-list">
						<view class="uni-media-list-body">
							<view class="uni-media-list-text-top info-box-tittle">{{ value.carNumber }}</view>
							<view class="info-box-context">
								<view class="info-box-context-name">编码</view>
								<view class="info-box-context-value">{{value.carCode}}</view>
							</view>
							<view class="info-box-context">
								<view class="info-box-context-name">名称</view>
								<view class="info-box-context-value">{{value.carName}}</view>
							</view>
	...
	export default {
		...
		methods: {
			selectValue: function(event) {
				...
				prevPage._data.dataForm.carInfoId = event.carInfoId;
				prevPage._data.dataForm.carInfoName = event.carNumber;
				//因为修改的是data里面的绑定数据，所以返回后页面数据会直接显示修改后的
				uni.navigateBack();
			},
			search(res) {
				uni.request({
					url: this.baseUrl + '/h5/carInfoSelect/' + res.value,
					data: {},
					header: {
						'token': this.getCookie('token')
					},
